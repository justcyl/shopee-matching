...
class CFG:
    ...
    ### BERT
    bert_model_name = "../input/shopee-models/paraphrase-multilingual-MiniLM-L12-v2"
...
def get_valid_embeddings(df, column, model, chunk=32):
    ...
    bert_embeddings = torch.zeros((df.shape[0], 384)).to(CFG.device)
    for i in tqdm(
...
# %%
def train(df, destination="oof_minilm_arcface", threshold=0.4):
    init()
...
def eval(test_df, destination="oof_minilm_arcface", threshold=0.4):
    init()
